...
def process_query(s, dictionary, le_mas):
    n = len(s)
    left, right = bisect_left(le_mas, n - 2), bisect_right(le_mas, n + 2)
    ...
    for i in range(left, right):
        ...
        if kscore < 3:
            ...
            if score == 1:
                return "{} 1 {}".format(s, word)
            if bscore > score:
                bscore = score
                bword = word
    
    if bscore < 3:
        mid = restore(s, bword)[0]
        return "{} 2 {} {}".format(s, mid, bword)
...
def main():
    ...
    with open('queries.txt', 'r') as file:
        ...
        queries = queries[j * 10000: (j + 1) * 10000]
    ...
    with open(f"answer{j}.txt", 'w') as file:
        file.write('\n'.join(res))

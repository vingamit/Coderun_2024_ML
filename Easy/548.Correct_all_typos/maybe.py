...
def get_correction(word: str, dictionary: List[str], diction: Set) -> Tuple[int, str]:
    if word in diction:
        return 0, ""
...
def process_query(query: str, dictionary: List[str], diction: Set) -> str:
    score, word = get_correction(query, dictionary, diction)
...
def main():
    ...
    for query in querys:
        s = process_query(query, dictionary, diction)
        res.append(s)
